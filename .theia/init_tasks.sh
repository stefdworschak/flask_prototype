...
echo "Creating the ${ADDITIONAL_MYSQL_USER} user in MySQL"
mysql -e "CREATE USER '${ADDITIONAL_MYSQL_USER}'@'%' IDENTIFIED BY '${ADDITIONAL_MYSQL_USER_PASSWORD}';" -u root
echo "Granting privileges"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO '${C9_USER}'@'%' WITH GRANT OPTION;" -u root
mysql -e "GRANT ALL PRIVILEGES ON *.* TO '${ADDITIONAL_MYSQL_USER}'@'%' WITH GRANT OPTION;" -u root
